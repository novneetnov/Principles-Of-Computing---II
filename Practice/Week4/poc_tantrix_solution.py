...
class Tantrix:
    ...
    def rotate_tile(self, index):
        ...
        """
        tile_code = self._tile_value[index]
        new_code = tile_code[-1] + tile_code[:-1]
    ...
    def get_neighbor(self, index, direction):
        ...
        """
        neighbour_offset = DIRECTIONS[direction]
        new_idx = tuple([index[dim] + neighbour_offset[dim] for dim in range(3)])
        return new_idx
        ...
        for tile_index in self._tile_value.keys():
            ...
            for direction in DIRECTIONS.keys():
                ...
                if self.tile_exists(neighbour_index):
                    ...
                    if tile_code[idx] != neighbour_code[reverse_direction(idx)]:
                        return False
                idx += 1
        return True
